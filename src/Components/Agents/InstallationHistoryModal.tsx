...
interface InstallationHistoryModalProps {
  isOpen: boolean;
  onClose: () => void;
  installationId: string | null;
}

const InstallationHistoryModal: React.FC<InstallationHistoryModalProps> = ({
  isOpen,
  onClose,
  installationId
}) => {
  const {
    ...
    installationId ? `/v1/installer/installation-history/` : null,
    isOpen,
    60000
  );
      ...
      <div className="bg-white h-full">
        ...
        <div className="flex flex-col w-full gap-4 p-4">
          <DataStateWrapper
            ...
            errorMessage="Failed to fetch installation details"
          >
            <div className="flex flex-col w-full gap-4">
              {/* Installation ID & Status Section */}
              ...
              <div className="flex flex-col p-2.5 gap-2 bg-white border-[0.6px] border-strokeGreyThree rounded-[20px]">
                <p className="flex gap-1 w-max text-textLightGrey text-xs font-medium pb-2">
                  <img src={locationIcon} alt="Location Icon" className="w-4 h-4" /> INSTALLATION DETAILS
                </p>
                <div className="flex items-center justify-between">
                  <Tag name="Google Address" />
                  <p className="text-xs font-bold text-textDarkGrey">
                    {data?.googleAddress}
                  </p>
                </div>
              ...
              <div className="flex flex-col p-2.5 gap-2 bg-white border-[0.6px] border-strokeGreyThree rounded-[20px]">
                <p className="flex gap-1 w-max text-textLightGrey text-xs font-medium pb-2">
                  <img src={lockIcon} alt="Device Icon" className="w-4 h-4" /> DEVICE DETAILS
                </p>
                <div className="flex items-center justify-between">
                  <Tag name="Device ID" />
                  <p className="text-xs font-bold text-textDarkGrey">
                ...
                <div className="flex items-center justify-between">
                  <Tag name="Token Status" />
                  <p className="text-xs font-bold text-textDarkGrey">
                    {data?.tokenStatus}
                  </p>
                </div>
                <div className="flex items-center justify-between">
                  <Tag name="Device Status" />
                  <p className="text-xs font-bold text-textDarkGrey">
                    {data?.deviceStatus}
                  </p>
                </div>
...
export default InstallationHistoryModal; 