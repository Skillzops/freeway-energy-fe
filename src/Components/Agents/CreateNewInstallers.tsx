...
interface CreateNewInstallersProps {
  isOpen: boolean;
...
const defaultInstallersFormData: InstallerFormData = {
  firstname: "",
  lastname: "",
  email: "",
  phoneNumber: "",
...
const CreateNewInstallers = ({
  isOpen,
  setIsOpen,
  refreshTable,
}: CreateNewInstallersProps) => {
  const { apiCall } = useApiCall();
  const [formData, setFormData] = useState<InstallerFormData>(
    defaultInstallersFormData
  );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const validatedData = installerSchema.parse(formData);
      ...
      await apiCall({
        ...
        method: "post",
        data: validatedData,
        successMessage: "Installer created successfully!",
      });

      await refreshTable();
      setIsOpen(false);
      setFormData(defaultInstallersFormData);
    } catch (error: any) {
      if (error instanceof z.ZodError) {
        ...
        const message =
          ...
          "Installer Creation Failed: Internal Server Error";
        setApiError(message);
      }
    } finally {
      setLoading(false);
  ...
  const isFormFilled = installerSchema.safeParse(formData).success;
...
export default CreateNewInstallers;
