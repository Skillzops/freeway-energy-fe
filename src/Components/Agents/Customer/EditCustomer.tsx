...
const EditNewCustomer = ({
  ...
  const handleIdImageChange = (file: File | null) => {
    setFormData((prev) => ({ ...prev, idImage: file || undefined }));
    resetFormErrors("idImage");
  };

  const handleContractFormImageChange = (file: File | null) => {
    setFormData((prev) => ({ ...prev, contractFormImage: file || undefined }));
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const isResubmit = mode === "resubmit" && customerId;

      await apiCall({
        endpoint: isResubmit ? `/v1/agents/${customerId}/resubmit` : "/v1/customers/create",
        method:  "post",
        data: formDataToSend,
        headers: { "Content-Type": "multipart/form-data" },
        successMessage: isResubmit ? "Customer resubmission sent" : "Customer created successfully!",
      });
      ...
      if (error instanceof z.ZodError) {
        ...
        const message =
          ...
          (mode === "resubmit"
            ? "Customer Resubmission Failed: Internal Server Error"
      ...
      <form className="flex flex-col items-center bg-white" onSubmit={handleSubmit} noValidate>
        ...
        >
          <h2 style={{ textShadow: "1px 1px grey" }} className="text-xl text-textBlack font-semibold font-secondary">
            {title ?? (mode === "resubmit" ? "Resubmit Customer" : "New Customer")}
          </h2>
        </div>
