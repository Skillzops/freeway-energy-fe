...
const ProductModal = ({
  ...
  const fetchSingleProduct = useGetRequest(
    `/v1/agents/product/${productID}`,
    false
  );
  ...
  const generateProductInventoryEntries = (data: any) => {
    const entries = data?.inventories?.map((item: any) => {
      return {
        ...
        quantity: item?.productInventoryQuantity,
        totalRemainingQuantities: item?.totalRemainingQuantities,
        totalInitialQuantities: item?.totalInitialQuantities,
  ...
  const inventoryData = useMemo(() => {
    return generateProductInventoryEntries(fetchSingleProduct?.data);
  }, [fetchSingleProduct]);
