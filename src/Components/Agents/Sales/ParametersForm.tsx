...
const ParametersForm = ({
  ...
  // Calculate initial payment when switching to installment mode or when duration changes
  useEffect(() => {
    if (formData.paymentMode === "INSTALLMENT" && formData.installmentDuration && formData.installmentDuration > 0) {
      // Calculate the monthly installment amount based on total cost and duration
      ...
      // Set the calculated amount as the initial payment
      setFormData(prev => ({
        ...prev,
        installmentStartingPrice: calculatedAmount
      }));
    } else if (formData.paymentMode === "INSTALLMENT") {
