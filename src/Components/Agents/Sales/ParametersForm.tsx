...
const ParametersForm = ({
...
}: {handleClose: () => void;currentProductId: string;}) => {
  ...
  const _effectiveMiscellaneousCosts =
  ...
  const handleSelectChange = (name: string, values: string | string[]) => {
    if (name === "paymentMode") {
      ...
      if (nextMode === "ONE_OFF") {
        ...
        const monthly =
        typeof product?.defaultMonthlyPayment === "number" &&
        product.defaultMonthlyPayment > 0 ?
        product.defaultMonthlyPayment :
        plan?.monthlyPayment ?? (
        duration > 0 ?
