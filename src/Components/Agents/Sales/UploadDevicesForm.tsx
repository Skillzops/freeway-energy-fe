...
import { FaPlus as _FaPlus } from "react-icons/fa";
import { MdFilterList, MdFilterListOff } from "react-icons/md";
import { HiPlus as _HiPlus } from "react-icons/hi2";
...
const UploadDevicesForm = observer(
  ({
  ...
  }: {handleClose: () => void;setDescription: React.Dispatch<React.SetStateAction<string>>;currentProductId: string;}) => {
    ...
    const [_prevDescription, setPrevDescription] = useState<string>("");
    ...
    const selectedLinkedDevices: DeviceResponse[] = (
      filteredDevices ??
      assignedDevicesData?.devices ??
      assignedDevicesData?.data?.devices ??
      []
    ).filter((device) => selectedDeviceIds && selectedDeviceIds.includes(device.id));
    ...
    const filterDevices = async () => {
      ...
      if (searchTerm.trim()) {
        if (filterKey && filterKey !== "search") newParams[filterKey] = searchTerm;else
        newParams.serialNumber = searchTerm;
      }
      return newParams;
    };
    ...
    const fetchDevice = async () => {
      ...
      try {
        ...
        const devices = response.data?.devices || response.data?.data?.devices || [];
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      const { name, value, type: _type, checked: _checked } = e.target;
      if (name === "searchTerm") {
        setSearchTerm(value);
        e.preventDefault();
      }
    ...
    const assignedDevices = assignedDevicesData?.devices ?? assignedDevicesData?.data?.devices ?? [];

    const devicesSource = filteredDevices ?? assignedDevices;
    ...
    return (
      <form className="flex flex-col justify-between h-full max-h-[400px] gap-2">
          ...
          <div className="flex flex-col items-center justify-center w-full max-h-[410px] mt-2 gap-1">
              ...
              productData?.inventories?.length > 1 ? "ies" : "y"} Below`
              }
                    </p>
                    ...
                    return (
                      <div
                        ...
                        <div
                          ...
                          "bg-slate-200 border-strokeGreyTwo"} cursor-pointer transition-colors`
                          }
                          onClick={() => {
          ...
          null)}
              ...
              {linkView === "" ? null : devicesSource.length === 0 ? null :
        ...
        <div className="flex flex-col w-full gap-1 px-5 pb-4 mt-4 absolute bottom-0 left-0">
            ...
            SaleStore.getTentativeDevicesByInventory(
              currentProductId,
              currentInventoryId
            )?.length || linkedDevicesCount} device${
